...
<div class="row g-3">
    ...
    <div class="col-md-3">
        ...
        <select class="form-select" id="lista_precio_id" name="lista_precio_id">
            ...
            @foreach ($listas as $lista)
                <option value="{{ $lista->id }}" @selected((int) old('lista_precio_id', $cotizacion->lista_precio_id) === $lista->id)>
                    {{ $lista->nombre }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-2">
        ...
        <input class="form-control @error('fecha') is-invalid @enderror" type="date" id="fecha" name="fecha"
               required value="{{ old('fecha', $cotizacion->fecha?->toDateString() ?? now()->toDateString()) }}">
    ...
    <div class="col-md-2">
        ...
        <input class="form-control @error('vigencia') is-invalid @enderror" type="date" id="vigencia" name="vigencia"
               value="{{ old('vigencia', $cotizacion->vigencia?->toDateString()) }}">
        <small class="text-muted">Pasada esta fecha ya no se acepta.</small>
    ...
    <div class="col-md-2">
        ...
        <input class="form-control @error('moneda') is-invalid @enderror" type="text" id="moneda" name="moneda"
               maxlength="3" required value="{{ old('moneda', $cotizacion->moneda ?? 'MXN') }}">
        @error('moneda') <div class="invalid-feedback">{{ $message }}</div> @enderror
    </div>

    <div class="col-md-10">
        <label class="form-label" for="notas">Notas</label>
        <textarea class="form-control" id="notas" name="notas" rows="2">{{ old('notas', $cotizacion->notas) }}</textarea>
    </div>
</div>

<div class="d-flex justify-content-end gap-2 mt-4">
    <a class="btn btn-outline-secondary" href="{{ route('ventas.cotizaciones.index') }}">Cancelar</a>
